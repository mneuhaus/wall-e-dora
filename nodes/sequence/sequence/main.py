...
def run_hands_up(node: Node):
    ...
    # Arms up (left=#2, right=#13), hold, small pulse, then back down (3–5s total)
    ...
    node.send_output("move_servo_sequence", pa.array([{ "id": 2, "position": max(0, ARM_LEFT_UP - 60) }]))
    node.send_output("move_servo_sequence", pa.array([{ "id": 13, "position": min(ARM_RIGHT_NEUTRAL, ARM_RIGHT_UP + 70) }]))
    time.sleep(0.4)
    node.send_output("move_servo_sequence", pa.array([{ "id": 2, "position": ARM_LEFT_UP }]))
    node.send_output("move_servo_sequence", pa.array([{ "id": 13, "position": ARM_RIGHT_UP }]))
    time.sleep(0.4)
...
def run_candy(node: Node):
    ...
    # Peek with a gentle pivot left-center
